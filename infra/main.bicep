...
var tags = { Application: environmentName}
...
module api './app/api.bicep' = {
  ...
  params: {
    ...
    runtimeName: functionAppRuntime
...
module storagePrivateEndpoint 'app/storage-PrivateEndpoint.bicep' = if (vnetEnabled) {
  ...
  params: {
    ...
    queuePrivateEndpointName: queuePrivateEndpointName
    tablePrivateEndpointName: tablePrivateEndpointName
  }
}

// Key Vault with Private Endpoint
module keyVault 'app/key-vault-PrivateEndpoint.bicep' = if (createKV) {
  name: 'keyVault'
  scope: rg
  params: {
    ...
    // If VNet is enabled, pass the networking details. Otherwise empty strings are fine (module handles condition).
    virtualNetworkId: vnetEnabled ? serviceVirtualNetwork.outputs.vnetId : ''
    subnetPrivateEndpointId: vnetEnabled ? serviceVirtualNetwork.outputs.peSubnetID : ''
    // Grant the Function App's identity access to the Key Vault
    principalId: apiUserAssignedIdentity.outputs.principalId
  }
}

// Service Bus (Basic Tier)
module serviceBus 'app/servicebus.bicep' = if (createServiceBus) {
  name: 'serviceBus'
  scope: rg
  params: {
    name: !empty(serviceBusName) ? serviceBusName : '${abbrs.serviceBusNamespaces}${resourceToken}'
    queueName: !empty(serviceBusQueueName) ? serviceBusQueueName : 'sbq-${resourceToken}'
    location: location
    tags: tags
    // Basic tier doesn't support VNet/Private Endpoint parameters
    principalId: apiUserAssignedIdentity.outputs.principalId
  }
}
